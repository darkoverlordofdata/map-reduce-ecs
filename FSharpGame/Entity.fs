...
let CreatePlayer (content:ContentManager, position) =
    let sprite = CreateSprite(Layer.PLAYER, content.Load<Texture2D>("images/fighter"))
    {
        Id = NextUniqueId();
        Name = "Player";
        Active = true;
...
let CreateBullet (content:ContentManager, position) =
    let sprite = CreateSprite(Layer.BULLET, content.Load<Texture2D>("images/bullet"))
    {
        Id = NextUniqueId();
        Name = "Bullet";
        Active = false;
...
let CreateExplosion (content:ContentManager, position:Vector2, scale:float32) =
    let sprite = CreateSprite(Layer.EXPLOSION, content.Load<Texture2D>("images/explosion"))
    {
        Id = NextUniqueId();
        Name = "Explosion";
        Active = false;
